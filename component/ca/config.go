...
func AddCertificate(certificate string) error {
	...
	defer mutex.Unlock()
	if certificate == "" {
		return fmt.Errorf("add certificate failed: certificate is empty")
	}
	certificateBytes := []byte(certificate)
	var block *pem.Block
	block, _ = pem.Decode(certificateBytes)
	if block == nil {
		return fmt.Errorf("add certificate failed: certificate block is nil")
	}
	if block.Type != "CERTIFICATE" || len(block.Headers) != 0 {
		return fmt.Errorf("add certificate failed: certificate block is invalid")
	}
	if cert, err := x509.ParseCertificate(block.Bytes); err == nil {
		trustCerts = append(trustCerts, cert)
		return nil
	} else {
		return fmt.Errorf("add certificate failed %s", err.Error())
	}
}
