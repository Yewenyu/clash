...
import (
	...
	"github.com/Dreamacro/clash/addons/metacubex/transport/socks5"
...
func (t *Trojan) StreamConn(ctx context.Context, conn net.Conn) (net.Conn, error) {
	...
	if len(t.option.ClientFingerprint) != 0 {
		if t.option.Reality == nil {
			utlsConn, valid := vmessMC.GetUTLSConn(conn, t.option.ClientFingerprint, tlsConfig)
			if valid {
...
func (t *Trojan) StreamWebsocketConn(ctx context.Context, conn net.Conn, wsOptions *WebsocketOption) (net.Conn, error) {
	...
	return vmessMC.StreamWebsocketConn(ctx, conn, &vmessMC.WebsocketConfig{
		Host:                     wsOptions.Host,
		Port:                     wsOptions.Port,
		Path:                     wsOptions.Path,
