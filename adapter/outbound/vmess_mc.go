...
import (
	...
	mihomoVMess "github.com/Dreamacro/clash/addons/metacubex/transport/vmess"
...
func (v *VmessMC) StreamConnContext(ctx context.Context, c net.Conn, metadata *C.Metadata) (net.Conn, error) {
	var err error
...
func (v *VmessMC) streamConn(c net.Conn, metadata *C.Metadata) (conn net.Conn, err error) {
	if metadata.NetWork == C.UDP {
		...
		destPort := uint16(metadata.DstPort)
		if N.NeedHandshake(c) {
			conn = v.client.DialEarlyConn(c,
				M.ParseSocksaddrHostPort(metadata.String(), destPort))
		} else {
...
func (v *VmessMC) DialContext(ctx context.Context, metadata *C.Metadata, opts ...dialer.Option) (_ C.Conn, err error) {
	// gun transport
	if v.transport != nil && len(opts) == 0 {
		...
		destPort := uint16(metadata.DstPort)
		c, err = v.client.DialConn(c, M.ParseSocksaddrHostPort(metadata.String(), destPort))
		if err != nil {
			return nil, err
		}
...
func (v *VmessMC) DialContextWithDialer(ctx context.Context, dialer dialer.Dialer, metadata *C.Metadata) (_ C.Conn, err error) {
...
func (v *VmessMC) ListenPacketContext(ctx context.Context, metadata *C.Metadata, opts ...dialer.Option) (_ C.PacketConn, err error) {
...
func (v *VmessMC) ListenPacketWithDialer(ctx context.Context, dialer dialer.Dialer, metadata *C.Metadata) (_ C.PacketConn, err error) {
	// vmess use stream-oriented udp with a special address, so we need a net.UDPAddr
	if !metadata.Resolved() {
		ip, err := resolver.ResolveIP(metadata.Host)
		if err != nil {
...
func (v *VmessMC) ListenPacketOnStreamConn(ctx context.Context, c net.Conn, metadata *C.Metadata) (_ C.PacketConn, err error) {
	// vmess use stream-oriented udp with a special address, so we need a net.UDPAddr
	if !metadata.Resolved() {
		ip, err := resolver.ResolveIP(metadata.Host)
		if err != nil {
	...
	return newPacketConn(&vmessPacketConnMC{Conn: c, rAddr: metadata.UDPAddr()}, v), nil
}

func (v *VmessMC) SupportUOT() bool {
	return true
}

func NewVmessMC(option VmessOptionMC) (*VmessMC, error) {
	...
	case "grpc":
		dialFn := func(network, addr string) (net.Conn, error) {
			var err error
			var cDialer dialer.Dialer = dialer.NewDialer(v.Base.DialOptions()...)
...
func (uc *vmessPacketConnMC) WriteTo(b []byte, addr net.Addr) (int, error) {
...
func (uc *vmessPacketConnMC) ReadFrom(b []byte) (int, net.Addr, error) {
