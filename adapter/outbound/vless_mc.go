...
import (
	...
	"github.com/Dreamacro/clash/addons/metacubex/common/convert"
	N "github.com/Dreamacro/clash/addons/metacubex/common/net"
	"github.com/Dreamacro/clash/addons/metacubex/common/utils"
	"github.com/Dreamacro/clash/addons/metacubex/component/ca"
	"github.com/Dreamacro/clash/addons/metacubex/component/proxydialer"
	tlsC "github.com/Dreamacro/clash/addons/metacubex/component/tls"
	"github.com/Dreamacro/clash/addons/metacubex/patch"
	gunMC "github.com/Dreamacro/clash/addons/metacubex/transport/gun"
	vlessMC "github.com/Dreamacro/clash/addons/metacubex/transport/vless"
	vmessMC "github.com/Dreamacro/clash/addons/metacubex/transport/vmess"
...
type Vless struct {
	*Base
	client *vlessMC.Client
	option *VlessOption

	// for gun mux
	gunTLSConfig *tls.Config
	gunConfig    *gunMC.Config
	transport    *gunMC.TransportWrap
...
func (v *Vless) StreamConnContext(ctx context.Context, c net.Conn, metadata *C.Metadata) (net.Conn, error) {
	...
	case "ws":
		...
		wsOpts := &vmessMC.WebsocketConfig{
			Host:                     host,
			Port:                     port,
		...
		c, err = vmessMC.StreamWebsocketConn(ctx, c, wsOpts)
	case "http":
		...
		c = vmessMC.StreamHTTPConn(c, httpOpts)
	case "h2":
		...
		h2Opts := &vmessMC.H2Config{
			Hosts: v.option.HTTP2Opts.Host,
			Path:  v.option.HTTP2Opts.Path,
		}

		c, err = vmessMC.StreamH2Conn(c, h2Opts)
	case "grpc":
		c, err = gunMC.StreamGunWithConn(c, v.gunTLSConfig, v.gunConfig, v.realityConfig)
	default:
		// default tcp network
		// handle TLS
		c, err = v.streamTLSConn(ctx, c, false)
...
func (v *Vless) streamTLSConn(ctx context.Context, conn net.Conn, isH2 bool) (net.Conn, error) {
	if v.option.TLS {
		...
		return vmessMC.StreamTLSConn(ctx, conn, &tlsOpts)
	}
...
func (v *Vless) DialContext(ctx context.Context, metadata *C.Metadata, opts ...dialer.Option) (_ C.Conn, err error) {
	...
	if v.transport != nil && len(opts) == 0 {
		log.Debugln("DialContext transport enter")
		c, err := gunMC.StreamGunWithTransport(v.transport, v.gunConfig)
		if err != nil {
			return nil, err
		}
...
//func (v *Vless) ListenPacketWithDialer(ctx context.Context, dialer C2.Dialer, metadata *C.Metadata) (_ C.PacketConn, err error) {
...
//	// vless use stream-oriented udp with a special address, so we need a net.UDPAddr
//	if !metadata.Resolved() {
//		ip, err := resolver.ResolveIPMeta(ctx, metadata.Host)
//		if err != nil {
//			return nil, errors.New("can't resolve ip")
//		}
//		metadata.DstIP = ip
...
func parseVlessAddr(metadata *C.Metadata, xudp bool) *vlessMC.DstAddr {
	var addrType byte
	var addr []byte
	switch metadata.AddrType() {
	case socks5.AtypIPv4:
		addrType = vlessMC.AtypIPv4
		addr = make([]byte, net.IPv4len)
		copy(addr[:], metadata.DstIP)
	case socks5.AtypIPv6:
		addrType = vlessMC.AtypIPv6
		addr = make([]byte, net.IPv6len)
		copy(addr[:], metadata.DstIP)
	case socks5.AtypDomainName:
		addrType = vlessMC.AtypDomainName
		addr = make([]byte, len(metadata.Host)+1)
		addr[0] = byte(len(metadata.Host))
		copy(addr[1:], metadata.Host)
	}

	return &vlessMC.DstAddr{
...
func NewVless(option VlessOption) (*Vless, error) {
	var addons *vlessMC.Addons
	if option.Network != "ws" && len(option.Flow) >= 16 {
		option.Flow = option.Flow[:16]
		switch option.Flow {
		case vlessMC.XRV:
			log.Warnln("To use %s, ensure your server is upgrade to Xray-core v1.8.0+", vlessMC.XRV)
			addons = &vlessMC.Addons{
				Flow: option.Flow,
			}
		case vlessMC.XRO, vlessMC.XRD, vlessMC.XRS:
	...
	client, err := vlessMC.NewClient(option.UUID, addons)
	if err != nil {
		return nil, err
	}
	...
	case "grpc":
		dialFn := func(network, addr string) (net.Conn, error) {
			var err error
			var cDialer dialer.IDialer = dialer.NewDialer(v.Base.DialOptions()...)
			if len(v.option.DialerProxy) > 0 {
		...
		v.transport = gunMC.NewHTTP2Client(dialFn, tlsConfig, v.option.ClientFingerprint, v.realityConfig)
	}
